...
function defaultAvatar(name){ return `/api/avatar/${encodeURIComponent(name || 'user')}`; }

app.get('/api/avatar/:name', (req,res)=>{
  const name = (req.params.name || 'U').slice(0,2).toUpperCase();
  res.type('svg').send(`<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160"><defs><linearGradient id="g" x1="0" x2="1"><stop stop-color="#ff77d9"/><stop offset="1" stop-color="#7c5cff"/></linearGradient></defs><rect width="100%" height="100%" rx="36" fill="url(#g)"/><text x="50%" y="55%" text-anchor="middle" font-size="56" font-family="Arial" fill="white" font-weight="700">${name}</text></svg>`);
});
...
app.post('/api/auth/register', async (req,res)=>{
  ...
  const exists = db.prepare('SELECT id FROM users WHERE provider=? AND username=?').get('local', username);
...
app.post('/api/auth/login', async (req,res)=>{
  const { username, password } = req.body;
  const user = db.prepare('SELECT * FROM users WHERE provider=? AND username=?').get('local', username);
...
app.get('/auth/discord/callback', async (req,res)=>{
  try{
    ...
    const avatar = du.avatar ? `https://cdn.discordapp.com/avatars/${du.id}/${du.avatar}.png?size=160` : defaultAvatar(du.global_name || du.username);
    let user = db.prepare('SELECT * FROM users WHERE discord_id=?').get(du.id);
    if(!user){
      const info = db.prepare('INSERT INTO users (public_id, provider, discord_id, username, avatar_url) VALUES (?,?,?,?,?)').run('u_'+nanoid(10), 'discord', du.id, du.global_name || du.username, avatar);
      user = db.prepare('SELECT * FROM users WHERE id=?').get(info.lastInsertRowid);
    } else {
      db.prepare('UPDATE users SET username=?, avatar_url=? WHERE id=?').run(du.global_name || du.username, avatar, user.id);
    }
...
app.get('/api/games/:pack', (req,res)=>{
  const pack = Number(req.params.pack) || 1;
...
app.post('/api/games/:id/reaction', requireAuth, (req,res)=>{
  ...
  db.prepare('INSERT INTO reactions (user_id, game_id, value) VALUES (?,?,?) ON CONFLICT(user_id, game_id) DO UPDATE SET value=excluded.value').run(req.user.id, req.params.id, value);
  io.emit('dataChanged'); res.json({ok:true});
});
app.post('/api/games/:id/vote', requireAuth, (req,res)=>{
  const choice = req.body.choice === 'no' ? 'no' : 'yes';
  db.prepare('INSERT INTO votes (user_id, game_id, choice) VALUES (?,?,?) ON CONFLICT(user_id, game_id) DO UPDATE SET choice=excluded.choice').run(req.user.id, req.params.id, choice);
  io.emit('dataChanged'); res.json({ok:true});
});
...
app.post('/api/admin/games', requireAdmin, (req,res)=>{
  const {pack, video_url, title, size, multiplayer, genre, description} = req.body;
  if(!title) return res.status(400).json({error:'Нужно название'});
  db.prepare('INSERT INTO games (pack, video_url, title, size, multiplayer, genre, description) VALUES (?,?,?,?,?,?,?)').run(Number(pack)||1, video_url||'', title, size||'', multiplayer||'', genre||'', description||'');
  io.emit('dataChanged'); res.json({ok:true});
});
app.put('/api/admin/games/:id', requireAdmin, (req,res)=>{
  const {pack, video_url, title, size, multiplayer, genre, description} = req.body;
  db.prepare('UPDATE games SET pack=?, video_url=?, title=?, size=?, multiplayer=?, genre=?, description=? WHERE id=?').run(Number(pack)||1, video_url||'', title||'', size||'', multiplayer||'', genre||'', description||'', req.params.id);
  io.emit('dataChanged'); res.json({ok:true});
});
app.delete('/api/admin/games/:id', requireAdmin, (req,res)=>{ db.prepare('DELETE FROM games WHERE id=?').run(req.params.id); io.emit('dataChanged'); res.json({ok:true}); });
