...
Если видео начинается, но потом зависает, сайт пытается сам продолжить или перезапустить фон. Если зависания все равно повторяются, лучше прогнать mp4 через `faststart`, не меняя качество:

```bash
ffmpeg -i anime-bg.mp4 -c copy -movflags +faststart anime-bg-faststart.mp4
```

После этого замени файл на:
